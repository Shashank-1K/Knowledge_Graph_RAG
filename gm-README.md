...
A **Retrieval-Augmented Generation (RAG)** system powered by a **Directed Knowledge Graph**, built with **LlamaIndex**, **Ollama**, **Memgraph**, and **Streamlit**. Instead of letting the LLM freely decide what to extract, this system uses **structured stakeholder lenses**, **organic relation discovery**, and **smart incremental building** to build a rich, queryable knowledge graph from your documents — all running **100% locally** with no API keys required.

---

## 📋 Table of Contents

* [What Makes This Different](https://www.google.com/search?q=%23-what-makes-this-different)
* [System Architecture](https://www.google.com/search?q=%23-system-architecture)
* [Complete Data Flow](https://www.google.com/search?q=%23-complete-data-flow)
* [Stakeholder Lenses](https://www.google.com/search?q=%23-stakeholder-lenses)
* [Organic Relation Discovery](https://www.google.com/search?q=%23-organic-relation-discovery)
...
* [How It Works — Detailed](https://www.google.com/search?q=%23-how-it-works--detailed)
* [Configuration](https://www.google.com/search?q=%23-configuration)
* [Memgraph Lab Queries](https://www.google.com/search?q=%23-memgraph-lab-queries)
* [Troubleshooting](https://www.google.com/search?q=%23-troubleshooting)
* [Performance Notes](https://www.google.com/search?q=%23-performance-notes)
* [Example Queries](https://www.google.com/search?q=%23-example-queries)
...
┌─────────────────────────────────────────────────────────────────┐
│                        STREAMLIT UI                             │
│  💬 Chat  │  🔍 Explorer  │  🔭 Lenses  │  📂 Manage Documents  │
└─────────────────────────────────────────────────────────────────┘
              │               │               │
              ▼               ▼               ▼
      ┌──────────────┐ ┌──────────────┐ ┌──────────────┐
      ...
      │  directed_extractor → Multi-lens LLM extraction │
      │  organic_relations  → Cross-chunk discovery     │
      │  incremental_builder→ Smart doc diffing/merging │
      │  doc_registry.py    → SHA-256 state tracking    │
      └─────────────────────────────────────────────────┘
...
### 1. Incremental Build Phase (Adding a Document)

```
New PDF / TXT / DOCX
      │
      ▼ DocRegistry Tracker (SHA-256 Hash Check)
┌───────────────────────────────────────────┐
│ Has this exact file been ingested before? │
│  - YES → Skip entirely (saves compute)    │
│  - NO  → Pass to extraction pipeline      │
└─────────────────────┬─────────────────────┘
                      │
                      ▼
┌─────────────────┐   ┌─────────────────────────────────┐
│  Text Chunks    │   │     STAKEHOLDER LENS EXTRACTION │
│  (TextNodes)    ├───►  🔵 Executive  → triples        │
└─────────────────┘   │  🟢 Technical  → triples        │
                      │  🟣 HR_People  → triples        │
                      │  🟠 Operations → triples        │
                      │  🟡 Client     → triples        │
                      └──────────────┬──────────────────┘
                                     ...
                                     ▼
                   ┌─────────────────────────────────┐
                   │    CROSS-DOC ORGANIC MERGE      │
                   │ 1. Fetch ALL existing triples   │
                   │ 2. Find links between old & new │
                   │    (Hubs, Co-occurrence)        │
                   └──────────────┬──────────────────┘
                                  │
                                  ▼
...
User Question
      ...
      │
      ▼
  Build context & synthesize via Gemma3 LLM
